...
public class LoanChargeReadPlatformServiceImpl implements LoanChargeReadPlatformService {
    ...
    public Collection<LoanChargeData> retrieveLoanChargesDueAsOn(final Long loanId, final LocalDate dueDate) {
        final LoanChargeMapper rm = new LoanChargeMapper();
        final String sql = "select " + rm.schema() + " where lc.loan_id=? AND (lc.due_for_collection_as_of_date IS NULL or lc.due_for_collection_as_of_date <= ?) AND lc.is_active = true AND lc.amount_outstanding_derived > 0 "
