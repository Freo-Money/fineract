...
public class ClientsApiResource {
    ...
    @Operation(summary = "Create a Client", description = "Note:\n\n"
            + "1. You can enter either: firstname/middlename/lastname - for a person (middlename and lastname are optional) OR fullname - for a business or organisation (or person known by one name).\n"
            + "\n" + "2.If address is enable(enable-address=true), then additional field called address has to be passed.\n\n"
            + "Mandatory Fields: firstname (and optionally lastname) OR fullname, officeId, active=true and activationDate OR active=false, if(address enabled) address\n\n"
            + "Optional Fields: groupId, externalId, accountNo, staffId, mobileNo, savingsProductId, genderId, clientTypeId, clientClassificationId, maritalStatusId")
    ...
    private Long getResolvedClientId(final Long clientId, final ExternalId clientExternalId) {
        ...
        if (resolvedClientId == null) {
            ...
            if (resolvedClientId == null) {
                throw new ClientNotFoundException(clientExternalId.getValue());
            }
        }
