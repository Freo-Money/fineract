    ...
    private static final class ChargeMapper implements RowMapper<ChargeData> {
        ...
        public String chargeSchema() {
            return "c.id as id, c.name as name, c.amount as amount, c.currency_code as currencyCode, "
                    ...
                    + "c.max_cumulative_penalty_cap as maxCumulativePenaltyCap, " + "c.penalty_wait_period as penaltyWaitPeriod, "
        ...
        public ChargeData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            final Integer penaltyWaitPeriod = JdbcSupport.getInteger(rs, "penaltyWaitPeriod");
            ...
            return ChargeData.builder().id(id).name(name).amount(amount).currency(currency).digitsAfterDecimal(digitsAfterDecimal)
                    .roundingMode(roundingModeEnum).chargeTimeType(chargeTimeType).chargeAppliesTo(chargeAppliesToType)
                    .chargeCalculationType(chargeCalculationType).chargePaymentMode(chargePaymentMode).feeOnMonthDay(feeOnMonthDay)
                    .feeInterval(feeInterval).penalty(penalty).active(active).freeWithdrawal(isFreeWithdrawal)
                    ...
                    .minCap(minCap).maxCap(maxCap).maxCumulativePenaltyCap(maxCumulativePenaltyCap).penaltyWaitPeriod(penaltyWaitPeriod)
                    .feeFrequency(feeFrequencyType).incomeOrLiabilityAccount(glAccountData).taxGroup(taxGroupData).build();
