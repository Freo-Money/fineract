...
public class OverdueDueAdvPrincipalInterestOverdueDuePenaltiesFeesOrderLoanRepaymentScheduleTransactionProcessorTest {
    ...
    private final LocalDate dueDate1 = transactionDate.minusDays(1); // overdue/due
    private final LocalDate dueDate2 = transactionDate.plusDays(10); // future (advance)
    ...
    private final Money seventeen = Money.of(MONETARY_CURRENCY, BigDecimal.valueOf(17));
    ...
    private final Money thirtyEight = Money.of(MONETARY_CURRENCY, BigDecimal.valueOf(38));
    private final Money thirtyFive = Money.of(MONETARY_CURRENCY, BigDecimal.valueOf(35));
    private final Money twentyNine = Money.of(MONETARY_CURRENCY, BigDecimal.valueOf(29));
    ...
    private OverdueDueAdvPrincipalInterestOverdueDuePenaltiesFeesOrderLoanRepaymentScheduleTransactionProcessor underTest;
    ...
    @BeforeEach
    public void setUp() {
        underTest = new OverdueDueAdvPrincipalInterestOverdueDuePenaltiesFeesOrderLoanRepaymentScheduleTransactionProcessor(
    ...
    @Test
    public void normalRepayment_PlusIAcrossAllInstallments_thenOverduePenaltiesFees_only() {
        // Installment 1 (overdue/due): P=2, I=3, Penalty=4, Fee=5
        ...
        // Installment 2 (future): P=6, I=7, Penalty=8, Fee=9 (should not be applied for advance penalties/fees)
        ...
        // Ordering validation:
        // P+I for inst1: principal(19) then interest(17)
        // P+I for inst2: principal(14) then interest(8)
        // Penalty for inst1 only (remaining=1)
        final org.mockito.InOrder inOrder = Mockito.inOrder(installment1, installment2);
        inOrder.verify(installment1).payPrincipalComponent(eq(transactionDate), org.mockito.ArgumentMatchers.refEq(nineteen));
        inOrder.verify(installment1).payInterestComponent(eq(transactionDate), org.mockito.ArgumentMatchers.refEq(seventeen));
        inOrder.verify(installment2).payPrincipalComponent(eq(transactionDate), org.mockito.ArgumentMatchers.refEq(fourteen));
        inOrder.verify(installment2).payInterestComponent(eq(transactionDate), org.mockito.ArgumentMatchers.refEq(eight));
        inOrder.verify(installment1).payPenaltyChargesComponent(eq(transactionDate), org.mockito.ArgumentMatchers.refEq(one));

        // Fees should not be paid (remaining=0)
        Mockito.verify(installment1, Mockito.never()).payFeeChargesComponent(eq(transactionDate), Mockito.any());

        // Future installment's penalties/fees should not be applied
    ...
    @Test
    public void normalRepayment_PlusIAcrossAllInstallments_thenPenaltiesFeesAcrossAllInstallments() {
        // Installment 1 (overdue/due): P=2, I=3, Penalty=4, Fee=5
        final LoanRepaymentScheduleInstallment installment1 = Mockito.spy(new LoanRepaymentScheduleInstallment(loan, 1, fromDate1, dueDate1,
                BigDecimal.valueOf(2), BigDecimal.valueOf(3), BigDecimal.valueOf(5), BigDecimal.valueOf(4), false, null, BigDecimal.ZERO));

        // Installment 2 (future): P=6, I=7, Penalty=8, Fee=9
        final LoanRepaymentScheduleInstallment installment2 = Mockito.spy(new LoanRepaymentScheduleInstallment(loan, 2, fromDate2, dueDate2,
                BigDecimal.valueOf(6), BigDecimal.valueOf(7), BigDecimal.valueOf(9), BigDecimal.valueOf(8), false, null, BigDecimal.ZERO));

        final List<LoanRepaymentScheduleInstallment> installments = List.of(installment1, installment2);

        // Payment large enough to clear P+I for both installments (P+I total = 18) then allocate all penalties/fees.
        final LoanTransaction loanTransaction = Mockito
        ...
        // P+I phase across all installments (installment order)
        inOrder.verify(installment1).payPrincipalComponent(eq(transactionDate), org.mockito.ArgumentMatchers.refEq(forty));
        inOrder.verify(installment1).payInterestComponent(eq(transactionDate), org.mockito.ArgumentMatchers.refEq(thirtyEight));
        inOrder.verify(installment2).payPrincipalComponent(eq(transactionDate), org.mockito.ArgumentMatchers.refEq(thirtyFive));
        inOrder.verify(installment2).payInterestComponent(eq(transactionDate), org.mockito.ArgumentMatchers.refEq(twentyNine));

        // Charges phase (Penalty then Fees) across all installments
